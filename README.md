...
├── vite.config.ts               # Vite + Electron 集成
├── tailwind.config.js
├── postcss.config.js
├── tsconfig.json
├── index.html
├── build/
│   ├── icon.png                 # 应用图标（学士帽+课程表+AI标）
│   └── icon.svg                 # 图标源文件
├── electron/
│   ├── main/                    # Electron 主进程
│   │   ├── index.ts             # 主进程入口，创建窗口
│   │   ├── ipc.ts               # IPC 处理器注册入口
│   │   ├── ai.ts                # 通义千问 API 调用（主进程中转）
│   │   └── pdf.ts               # PDF 转图片（pdfjs-dist）
│   └── preload/
...
│   ├── utils/
│   │   ├── prompts.ts           # 两条核心提示词常量
│   │   ├── storage.ts           # localStorage 持久化 + 课节时间配置
│   │   ├── freeTime.ts          # 空闲时段算法 + 动态课节时间生成
│   │   └── icsExport.ts         # .ics 导出
│   └── components/
│       ├── WeekView.vue         # 周课表视图
│       ├── DayView.vue          # 单日课表视图
│       ├── CourseDialog.vue     # 课程增删改弹窗
│       ├── AiImport.vue         # AI 一键导入
│       ├── FreeTimePlanner.vue  # 空闲自习规划
...
1. **课表解析提示词**：让 AI 输出标准课程 JSON 数组
2. **空闲自习规划提示词**：让 AI 按空闲时段分段生成学习计划
